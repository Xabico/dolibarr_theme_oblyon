	...
	if ($mainmenu)
	{
		...
		if ($mainmenu == 'commercial')
		{
            ...
            if (! empty($conf->commande->enabled))
            {
                ...
                if (! empty($conf->expedition->enabled)) $newmenu->add("/commande/list.php?leftmenu=orders&viewstatut=2", $langs->trans("StatusOrderOnProcessShort"), 2, $user->rights->commande->lire);
		...
		if ($mainmenu == 'accountancy')
		{
			...
			if (! empty($conf->tax->enabled))
			{
				...
				if (empty($conf->global->TAX_DISABLE_VAT_MENUS))
				{
					...
					if($mysoc->country_code=='ES' && (isset($mysoc->localtax2_assuj) && $mysoc->localtax2_assuj=="1"))
					{
						...
						$newmenu->add("/compta/localtax/card.php?leftmenu=tax_vat&action=create",$langs->trans("NewPayment"),2,$user->rights->tax->charges->creer);
			...
			if (! empty($conf->comptabilite->enabled) && ($conf->global->MAIN_FEATURES_LEVEL >= 2))
			{
				...
				$newmenu->add("/compta/ventilation/list.php",$langs->trans("ToDispatch"),1,$user->rights->compta->ventilation->lire);
